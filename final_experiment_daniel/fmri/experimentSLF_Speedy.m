function myTrials=experimentSLF_Speedy(sub, run, test, recogTask)
% To run, type eg., myTrials=experimentSLF_Speedy(01, 1, false, true) in the command line
...
if run==1
    %% Load stimuli
    load stimuli_experiment.mat %CHANGED
    if test
        stimuli = stimuli(101:124);
    end
    numTrials=length(stimuli)/runs; %numb of trials per run
    %% Create structure with sentences and unique ids for each sentence
    for k=1:numTrials*runs
        myTrialsAll(k).sentence=stimuli(k);
        myTrialsAll(k).id=k;
    end
    %% Unique randomization for each participant    TODO: take 1 from each category.
    for ii=1:6:length(stimuli)
        myTrialsShuffled(ii:ii+5)=Shuffle(myTrialsAll(ii:ii+5));
    end
    save(['myTrialsShuffled' num2str(sub) '.mat'],'myTrialsShuffled','numTrials') %saves shuffled stimuli.
    myTrials = myTrialsShuffled(run:6:end);
    myTrials = Shuffle(myTrials);
else
    myTrialsShuffled = load(['myTrialsShuffled' num2str(sub) '.mat']);
    numTrials = myTrialsShuffled.numTrials;
    myTrialsShuffled = myTrialsShuffled.myTrialsShuffled;
    if run == 2
        myTrials = myTrialsShuffled(run:6:end);
        myTrials = Shuffle(myTrials);
    elseif run == 3
...
trigger = '5%'
KbReleaseWait
[keyPressed keyTime allKeys]=KbCheck;
...
not_trigger = [[1:length(allKeys)] == KbName(trigger)] == 0;
RestrictKeysForKbCheck(not_trigger);
%% show stimuli
for k=1:numTrials
...
if recogTask
    ...
    DrawFormattedText(win,'Premi il tasto a DESTRA se HAI VISTO la frase nell`ultima sezione \n ed il tasto a SINISTRA se NON l`HAI VISTO nell`ultima sezione. \n ------------------------------------ \n \n Le frasi appariranno dopo il simbolo rosso +.','center','center', [0,0,0], 50);
    Screen(win,'flip')
    pause(12); %time showing the instructions
    %% Load and organize data
    load distractors2.mat % todo add unseen sentences.
    ...
    if run==1
        %% Create structure with sentences and unique ids for each sentence
        for k=1:numTrialsTask*runs
            myTrialsAllTask(k).sentence=distractors(k);
            myTrialsAllTask(k).id=k;
        end
        %% Unique randomization for each participant    TODO: take 1 from each category.
        myTrialsShuffledTask=Shuffle(myTrialsAllTask);% randomise
        save(['myTrialsShuffledTask' num2str(sub) '.mat'],'myTrialsShuffledTask','numTrialsTask') %saves shuffled stimuli.
        %% for every run
        myTrialsTaskUnseen0 = myTrialsShuffledTask(1:numTrialsTask); %subselection of 4 stimuli for this run
        ...
        myTrialsShuffledTask = load(['myTrialsShuffledTask' num2str(sub) '.mat']);
        myTrialsTaskSeen0 = Shuffle([myTrials().sentence]'); %take random sentences out of last run (seen sentences).
        myTrialsTaskSeen  =  myTrialsTaskSeen0(1:numTrialsTask); %Choose 4. This doesn't change from run to run, because we take 4 from myTrials which is generated in each run.
        for k=1:numTrialsTask
            myTrialsTask(k).sentence=myTrialsTaskSeen(k);
            myTrialsTask(k).correctResponse=0;
        end
        if run == 2
            myTrialsTaskUnseen0 = myTrialsShuffledTask.myTrialsShuffledTask(numTrialsTask+1:numTrialsTask*2); %subselection of 4 stimuli for this run
            myTrialsTaskUnseen = [myTrialsTaskUnseen0().sentence]'; %extract sentences only
            for k=1:numTrialsTask
                myTrialsTask(k+numTrialsTask).sentence=myTrialsTaskUnseen(k);
                myTrialsTask(k+numTrialsTask).correctResponse=1;
            ...
            myTrialsTask = Shuffle(myTrialsTask);
        elseif run == 3
            myTrialsTaskUnseen0 = myTrialsShuffledTask.myTrialsShuffledTask(numTrialsTask*2+1:numTrialsTask*3); %subselection of 4 stimuli for this run
            myTrialsTaskUnseen = [myTrialsTaskUnseen0().sentence]'; %extract sentences only
            for k=1:numTrialsTask
                myTrialsTask(k+numTrialsTask).sentence=myTrialsTaskUnseen(k);
                myTrialsTask(k+numTrialsTask).correctResponse=1;
            ...
            myTrialsTask = Shuffle(myTrialsTask);
        elseif run == 4
            myTrialsTaskUnseen0 = myTrialsShuffledTask.myTrialsShuffledTask(numTrialsTask*3+1:numTrialsTask*4); %subselection of 4 stimuli for this run
            myTrialsTaskUnseen = [myTrialsTaskUnseen0().sentence]'; %extract sentences only
            for k=1:numTrialsTask
                myTrialsTask(k+numTrialsTask).sentence=myTrialsTaskUnseen(k);
                myTrialsTask(k+numTrialsTask).correctResponse=1;
            ...
            myTrialsTask = Shuffle(myTrialsTask);
        elseif run == 5
            myTrialsTaskUnseen0 = myTrialsShuffledTask.myTrialsShuffledTask(numTrialsTask*4+1:numTrialsTask*5); %subselection of 4 stimuli for this run
            myTrialsTaskUnseen = [myTrialsTaskUnseen0().sentence]'; %extract sentences only
            for k=1:numTrialsTask
                myTrialsTask(k+numTrialsTask).sentence=myTrialsTaskUnseen(k);
                myTrialsTask(k+numTrialsTask).correctResponse=1;
            ...
            myTrialsTask = Shuffle(myTrialsTask);
        elseif run == 6
            myTrialsTaskUnseen0 = myTrialsShuffledTask.myTrialsShuffledTask(numTrialsTask*5+1:numTrialsTask*6); %subselection of 4 stimuli for this run
            myTrialsTaskUnseen = [myTrialsTaskUnseen0().sentence]'; %extract sentences only
            for k=1:numTrialsTask
                myTrialsTask(k+numTrialsTask).sentence=myTrialsTaskUnseen(k);
                myTrialsTask(k+numTrialsTask).correctResponse=1;
