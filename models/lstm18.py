...
from keras.layers import Dropout, Embedding, LSTM, Dense, Input, Flatten, Lambda
...
def get_output(model, layer_name, batch_size=512, Xtest = None, hidden_layer=False, intermediate_timesteps=False):
    intermediate_layer_model = Model(inputs=model.input,outputs=model.get_layer(layer_name).output)
    layer_output_all = intermediate_layer_model.predict(Xtest, batch_size=batch_size, verbose=0)
    if hidden_layer:
        layer_output_last = layer_output_all[:,-1,:] # get last time step   4000x26x250 to 4000x250
        layer_output_last = pd.DataFrame(layer_output_last)
    #     # layer_output = np.reshape(layer_output,(layer_output.shape[0],layer_output.shape[1]*layer_output.shape[2]))
        if intermediate_timesteps:
            layer_output_3 = layer_output_all[:, 2, :]  # get last time step   4000x26x250 to 4000x250
            layer_output_9 = layer_output_all[:, 8, :]  # get last time step   4000x26x250 to 4000x250
            layer_output_15 = layer_output_all[:, 14, :]  # get last time step   4000x26x250 to 4000x250
            # timestep_outputs = {'word3': pd.DataFrame(layer_output_3),'word9': pd.DataFrame(layer_output_9),'word15': pd.DataFrame(layer_output_15)}
            layer_output_21 = layer_output_all[:, 20, :]
            layer_output_27 = layer_output_all[:, 26, :]
            layer_output_33 = layer_output_all[:, 32, :]
            timestep_outputs = {'word3': pd.DataFrame(layer_output_3),
                                'word9': pd.DataFrame(layer_output_9),
                                'word15': pd.DataFrame(layer_output_15),
...
def lstm(batch_size, drop, lstm_neuron, dense_final_neuron, activation, second_lstm, optimizer):
    inputs = Input(shape=(sequence_length,), dtype='int32')
    embedding = Embedding(len(word_index) + 1, embedding_dim, weights=[embedding_matrix], input_length=sequence_length,
                          trainable=True, mask_zero=True)(inputs)
    dropout0 = Dropout(drop)(embedding)
    lstm1 = LSTM(lstm_neuron, return_sequences=True, name='lstm_1', activation=activation)(dropout0)
    dropout1 = Dropout(drop)(lstm1)
    if second_lstm == 'yes':
        lstm2 = LSTM(lstm_neuron, return_sequences=True, name='lstm_2', activation=activation)(dropout1)
        dropout2 = Dropout(drop)(lstm2)
        dense_final = Dense(dense_final_neuron, name='dense_final', activation=activation)(dropout2)
    else:
        dense_final = Dense(dense_final_neuron, name='dense_final', activation=activation)(dropout1)
    x = Lambda(lambda x: x, output_shape=lambda s: s)(dense_final)
    flatten_final = Flatten()(x)
    dropout3 = Dropout(drop)(flatten_final)
    softmax_final = Dense(units=len(categories), activation='softmax', name='softmax_final')(dropout3)
...
drop = 0.3
lstm_neuron = 600
dense_final_neuron = 512
activation = 'elu'
second_lstm = 'yes' #TODO: here I added yes to see performance.
optimizer = 'Adam'

model = lstm(batch_size, drop, lstm_neuron, dense_final_neuron, activation, second_lstm, optimizer)

if save_checkpoints:
    filepath = path_to_dir + "weights-improvement-{epoch:02d}-{val_acc:.6f}.hdf5"  # https://machinelearningmastery.com/check-point-deep-learning-models-keras/
...
# # Save output_layers
#
# lstm_1 = get_output(model, 'lstm_1', batch_size = 512, Xtest=Xtest_encoded, hidden_layer=True, intermediate_timesteps=False)
# # lstm_2 = get_output(model, 'lstm_2', hidden_layer=True,Xtest=Xvalidation_encoded)
# # lstm_3 = get_output(model, 'lstm_3', Xtest=Xvalidation_encoded)
# # dense_1 = get_output(model, 'dense_1', Xtest=X_encoded,hidden_layer=True)
# # dense_2 = get_output(model, 'dense_2', Xtest=X_encoded,hidden_layer=True)
# dense_final = get_output(model, 'dense_final', Xtest=Xtest_encoded,hidden_layer=True)
# softmax_final = get_output(model, 'softmax_final', Xtest=Xtest_encoded,hidden_layer=False)
#
# np.savez_compressed(path_to_dir + 'output_layers_lstm1.npz', a=lstm_1)
# # np.savez_compressed(path_to_dir + 'output_layers_whole_dataset_b.npz', a=dense_1)
# # np.savez_compressed(path_to_dir + 'output_layers_whole_dataset_c.npz', a=dense_2)
# np.savez_compressed(path_to_dir + 'output_layers_dense_final.npz', a=dense_final)
# np.savez_compressed(path_to_dir + 'output_layers_softmax_final.npz', a=softmax_final)
#
#
...
#
#
# # np.savez_compressed(path_to_dir+'output_layers.npz', a=lstm_1,b=lstm_2, c=lstm_3, d=dense_final)
# # np.savez_compressed(path_to_dir+'output_layers.npz', a=lstm_1,b=lstm_2, c=dense_1, d=dense_2, e=dense_final)
# # np.savez_compressed(path_to_dir+'output_layers.npz', a=lstm_1,b=dense_1, c=dense_2, d=dense_final)
#
#
#
...
#
# amount_of_sentences_per_category = int(len(Xtest) / len(categories))
#
# if plot_RSA:
...
#     #              amount_sent=int(conv_1.shape[0] / len(categories)))
#     rsa.plot_RSA(path_to_dir, categories, layer=lstm_1, layer_name='lstm_1', amount_sent=amount_of_sentences_per_category)
#     rsa.plot_RSA(path_to_dir, categories, layer=dense_final, layer_name='dense_final',
#                  amount_sent=amount_of_sentences_per_category )
...
# n = 5
# layers = [lstm_1, dense_final]
# layer_names = ['lstm_1','dense_final']
#
...
#
#
# # Get vectors at each time step for stimuli
#
# stimuli_survey = pd.read_csv('final_stimuli_survey3.csv', encoding='utf-8-sig')
# ids = list(stimuli_survey['ids'])
#
# lstm_1,timestep_outputs = get_output(model, 'lstm_1', hidden_layer=True, Xtest=X_encoded[ids], intermediate_timesteps=True)
#
# for i in range(3,38,6):
#     np.savez_compressed(path_to_dir + 'intermediate_timesteps_stimuli'+'word'+str(i)+'.npz', a=timestep_outputs.get('word'+str(i)))